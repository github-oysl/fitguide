...
(async () => {
  ...
  check('全部文本资源可压缩且内容无损', async () => {
    const files = ['index.html', 'style.css', 'plans.js', 'guide.js', 'navigation.js', 'training-records.js', 'exercise-learning.js', 'dashboard.js', 'data.js', 'extra-data.js', 'checkin-stats.js', 'training.js', 'free-activity.js', 'surprise.js', 'manifest.webmanifest', 'comparison/comparison.css'];
