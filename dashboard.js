(function () {
  ...
  function navigate() {
    ...
    if (hash !== 'plans' && hash !== 'main') window.scrollTo({ top: 0, behavior: 'instant' });
  }
  ...
  function renderDay(date, dates, now, className = '') {
    ...
    return `<button type="button" class="calendar-day ${className} ${count ? 'has-checkin' : ''} ${value === stats.key(now) ? 'is-today' : ''}" data-date="${value}" ${future ? 'disabled' : ''} aria-pressed="${selected === value}" aria-label="${value}，${future ? '尚未到来' : count ? `已完成 ${count} 个动作` : '无打卡'}"><span>${date.getDate()}</span><i aria-hidden="true">${count ? '✓' : ''}</i></button>`;
  }
  function renderSelection(dates) {
    ...
    root.innerHTML = `<b>${escape(selected)} · ${ids.length ? `完成 ${ids.length} 个动作` : '没有打卡记录'}</b>${ids.length ? `<p>${ids.map(id => escape(byId.get(id)?.name || id)).join('、')}</p>` : '<p>休息也是训练的一部分。</p>'}`;
  }
  function render() {
    const now = new Date(), dates = stats.aggregate(done, now, new Set(byId.keys()));
    ...
    document.getElementById('stats-overview').innerHTML = [['week', '本周'], ['month', '本月'], ['year', '今年']].map(([type, label], index) => {
      ...
      return `<button type="button" class="overview-card" data-overview="${type}"><span>${label}打卡<span aria-hidden="true">↗</span></span><strong>${count.days}<small>天</small></strong><span class="overview-caption">${count.exercises} 次动作完成<span class="overview-index" aria-hidden="true">0${index + 1}</span></span></button>`;
    }).join('');
    ...
    document.getElementById('period-summary').innerHTML = `<div><b>${counts.days}</b><span>打卡天数</span></div><div><b>${counts.exercises}</b><span>完成动作次数</span></div><div><b>${stats.longestStreak(dates, bounds)}</b><span>周期内最长连续天数</span></div>`;
    ...
    document.getElementById('chart-caption').textContent = period === 'year' ? '柱高表示当月打卡天数占比 · 点击月份查看月历' : counts.days ? '绿色表示已打卡 · 点击日期查看当天动作' : '这段时间还没有打卡。完成动作后，这里会自动留下记录。';
